...
from donkeycar.utils import linear_bin, normalize_image
...
class KerasPilot(ABC):
    ...
    def run(
        self, img_arr: np.ndarray, other_arr: List[float] = None
    ...
    def inference(
        self, img_arr: np.ndarray, other_arr: Optional[np.ndarray]
    ) -> Tuple[Union[float, np.ndarray], ...]:
    ...
    def interpreter_to_output(
        self, interpreter_out: Sequence[Union[float, np.ndarray]]
    ) -> Tuple[Union[float, np.ndarray], ...]:
    ...
    def y_transform(
        self, record: Union[TubRecord, List[TubRecord]]
        ...
        dictionary keys."""
        raise NotImplementedError(f"{self} not ready yet for new training pipeline")
...
class KerasCategorical(KerasPilot):
    ...
    def interpreter_to_output(self, interpreter_out):
        ...
        throttle = dk.utils.linear_unbin(
            throttle_binned, N=N, offset=0.0, R=self.throttle_range
        )
...
class KerasLinear(KerasPilot):
    ...
    def interpreter_to_output(self, interpreter_out):
        ...
        brake = interpreter_out[2]  # ! 編集済み
        return steering[0], throttle[0], brake[0]  # ! 編集済み
    ...
    def y_transform(
        ...
        angle: float = record.underlying["user/angle"]
        throttle: float = record.underlying["user/throttle"]
        brake: float = record.underlying["user/brake"]  # ! 編集済み
        return {
            "n_outputs0": angle,
            "n_outputs1": throttle,
            "n_outputs2": brake,  # ! 編集済み
        }
    ...
    def output_shapes(self):
        ...
        shapes = (
            {"img_in": tf.TensorShape(img_shape)},
            {
                ...
                "n_outputs2": tf.TensorShape([]),  # ! 編集済み
            },
        )
        return shapes
...
class KerasMemory(KerasLinear):
    ...
    def x_transform(
        ...
        mem = [
            [r.underlying["user/angle"], r.underlying["user/throttle"]]
            for r in record[:-1]
        ]
    ...
    def y_transform(
        self, records: Union[TubRecord, List[TubRecord]]
    ) -> Dict[str, Union[float, List[float]]]:
    ...
    def __str__(self) -> str:
        """For printing model initialisation"""
        return super().__str__() + f"-L:{self.mem_length}-D:{self.mem_depth}"
...
class KerasIMU(KerasPilot):
    ...
    def output_shapes(self):
        ...
        shapes = (
            ...
            {"out_0": tf.TensorShape([]), "out_1": tf.TensorShape([])},
        )
        return shapes
...
class KerasBehavioral(KerasCategorical):
    ...
    def create_model(self):
        return default_bhv(
            num_bvh_inputs=self.num_behavior_inputs, input_shape=self.input_shape
        )
...
class KerasLocalizer(KerasPilot):
    ...
    def create_model(self):
        return default_loc(
            num_locations=self.num_locations, input_shape=self.input_shape
        )
...
class Keras3D_CNN(KerasPilot):
    ...
    def create_model(self):
        return build_3d_cnn(
            self.input_shape, s=self.seq_length, num_outputs=self.num_outputs
        )
...
class KerasLatent(KerasPilot):
    ...
    def compile(self):
        ...
        self.interpreter.compile(
            optimizer=self.optimizer, loss=loss, loss_weights=weights
        )
...
def default_memory(input_shape=(120, 160, 3), mem_length=3, mem_depth=0):
    drop = 0.2
    drop2 = 0.1
    logger.info(f"Creating memory model with length {mem_length}, depth {mem_depth}")
    img_in = Input(shape=input_shape, name="img_in")
    x = core_cnn_layers(img_in, drop)
    ...
    outputs = [
        Dense(1, activation=activation[i], name="n_outputs" + str(i))(x)
        for i in range(2)
    ]
...
def default_categorical(input_shape=(120, 160, 3)):
    ...
    model = Model(
        inputs=[img_in], outputs=[angle_out, throttle_out], name="categorical"
    )
    return model
...
def default_bhv(num_bvh_inputs, input_shape):
    ...
    model = Model(
        inputs=[img_in, bvh_in], outputs=[angle_out, throttle_out], name="behavioral"
    )
    return model
...
def default_loc(num_locations, input_shape):
    ...
    model = Model(
        inputs=[img_in], outputs=[angle_out, throttle_out, loc_out], name="localizer"
    )
    return model
